...
M.defaults = {
	...
	-- Buffer-local keymaps wired up on topic .md files inside _database/.
	-- Both fire in normal and insert mode. Set either to false to disable.
	-- Customize in your setup(), e.g.
	--   require("biblio").setup({
	--     insert_keymaps = { paper = "<C-r>", topic = "<C-l>" },
	--   })
	insert_keymaps = {
		paper = "<C-p>", -- insert a paper link  [Title](/papers/slug/)
		topic = "<C-t>", -- insert a topic link  [[Topic Title]]
	},
...
function M.resolve_blog_root(provided)
	local function looks_like_repo(p)
		...
		return vim.fn.isdirectory(p .. "/_database") == 1 and vim.fn.isdirectory(p .. "/_papers") == 1
	end

	if provided then
		provided = vim.fn.expand(provided)
		if looks_like_repo(provided) then
			return provided
		end
	end

	local cwd = vim.fn.getcwd()
	if looks_like_repo(cwd) then
		return cwd
	end
