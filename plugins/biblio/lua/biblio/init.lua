...
local function setup_autocmds()
	...
	vim.api.nvim_create_autocmd({ "BufRead", "BufNewFile" }, {
		...
		callback = function(ev)
			...
			if path:sub(1, #db + 1) ~= (db .. "/") then
				return
			end

			local km = cfg.options.insert_keymaps or {}

			if km.paper then
				vim.keymap.set({ "n", "i" }, km.paper, function()
			...
			if km.topic then
				vim.keymap.set({ "n", "i" }, km.topic, function()
					require("biblio.topic_ref").pick_and_insert()
				end, {
					buffer = ev.buf,
					silent = true,
					nowait = true,
					desc = "biblio: insert topic wikilink",
				})
			end
		end,
	})
...
function M.serve(opts)
	require("biblio.serve").start(opts)
end
function M.serve_stop()
	require("biblio.serve").stop()
end
function M.serve_status()
	require("biblio.serve").status()
end
function M.preview(port)
	require("biblio.serve").open_browser(port)
end

-- ─── git ────────────────────────────────────────────────────────────────────

function M.commit(opts)
	require("biblio.git").prompt_commit(opts)
end
function M.push()
	require("biblio.git").push()
end
