...
local TAG_HLS = {
  "PhysNavTagC1","PhysNavTagC2","PhysNavTagC3",
  "PhysNavTagC4","PhysNavTagC5","PhysNavTagC6",
}
...
local function is_open()
  if not state.wins or vim.tbl_isempty(state.wins) then return false end
...
local function pad_vertical(pane, lines, hl_ops)
  ...
  if n == 0 then return lines, hl_ops end
  local padded = {}
  for _ = 1, n do table.insert(padded, "") end
...
local function compute_geometry(cfg)
  ...
  -- space for content.
  if body_h < 12 then body_h = 12 end
  ...
  return {
    search  = { row = outer_row, col = outer_col,
                width = outer_w, height = search_h },
    tags    = { row = outer_row + search_h + gap_v, col = outer_col,
                width = tags_w,    height = body_h },
    list    = { row = outer_row + search_h + gap_v,
                col = outer_col + tags_w + gap_h,
                width = list_w,    height = body_h },
    preview = { row = outer_row + search_h + gap_v,
                col = outer_col + tags_w + gap_h + list_w + gap_h,
                width = preview_w, height = body_h },
    hints   = { row = outer_row + search_h + gap_v + body_h + gap_v,
                col = outer_col,
  ...
  for _, p in ipairs(state.projects) do
    ...
    cb_map[name] = function(s)
      ...
      vim.schedule(function()
        if is_open() then M._render_list(); M._render_preview() end
      end)
    end
  end
...
function M._render_tags()
  ...
  table.insert(lines, string.format(" %s All  (%d)",
    all_mark, #state.projects))

  for _, tag in ipairs(state.all_tags) do
    local cnt = 0
    for _, p in ipairs(state.projects) do
      for _, t in ipairs(p.tags or {}) do
        if t == tag then cnt = cnt + 1; break end
      end
    end
  ...
  if state.wins.tags and api.nvim_win_is_valid(state.wins.tags) then
    ...
    if state.mode == "tags" then
      ...
      if tc > #state.all_tags then tc = #state.all_tags end
      row = 2 + tc
    else
...
function M._render_list()
  ...
  if #state.filtered == 0 then
    ...
    for _, p in ipairs(state.filtered) do
      ...
      local lec_str = (p.lec_count and p.lec_count > 0)
                      and string.format(" x%d", p.lec_count) or ""
      ...
      local row    = string.format("  %s %s%s%s", pdf_str,
  ...
  for i, p in ipairs(state.filtered) do
    ...
    if not badge_s then badge_s, badge_e = line:find("%[tex%]") end
    if badge_s then
      add_hl("list", p.type == "typst" and "PhysNavTypst" or "PhysNavLatex",
             ln, badge_s - 1, badge_e)
    end

    local lec_s, lec_e = line:find(" x%d+", badge_e or 1)
    if lec_s then add_hl("list", "PhysNavLecCount", ln, lec_s - 1, lec_e) end

    local gs = state.git_status[p.name]
    if gs and gs ~= "" then
      local gi = line:find(gs, (lec_e or badge_e or 1) + 1, true)
      if gi then add_hl("list", "PhysNavGitDirty", ln, gi - 1, gi - 1 + #gs) end
    end
    ...
    add_hl("list",
      p.category == "EPFL_lecture" and "PhysNavCatEPFL" or "PhysNavCatNotes",
      ln, cc_start, cc_end)
    ::continue::
  end
...
function M._render_preview()
  ...
  local function push_hl(g, l, s, e) table.insert(hl_ops, {g, l, s, e}) end
  ...
  if not sel then
    ...
    local title = string.format(" %s  %s", type_badge,
                                trunc(sel.name, inner_w - 6))
    table.insert(lines, title)
    push_hl(sel.type == "typst" and "PhysNavTypst" or "PhysNavLatex",
            #lines - 1, 1, 4)
    ...
    for _, f in ipairs(fields) do
      ...
      elseif key == "git" then
        push_hl((gs_val and gs_val ~= "") and "PhysNavGitDirty" or "PhysNavGitClean",
                ln, val_start, -1)
      elseif key == "lec" then
        push_hl("PhysNavLecCount", ln, val_start, -1)
      elseif key == "cat" then
        push_hl(sel.category == "EPFL_lecture" and "PhysNavCatEPFL" or "PhysNavCatNotes",
                ln, val_start, -1)
...
function M._render_hints()
  ...
  if #state.active_tags > 0 then
    local logic = (state.tag_multi and #state.active_tags > 1)
      and (state.tag_and and "AND" or "OR")
      or  "="
    table.insert(suffix_parts,
      logic .. ":" .. table.concat(state.active_tags, "+"))
  end
...
local function apply_titles()
  local function set_title(pane, title, is_active)
    local w = state.wins[pane]
    if not w or not api.nvim_win_is_valid(w) then return end
  ...
  if #state.active_tags > 0 then
    tag_title = string.format(" Tags [%s:%d] ",
      state.tag_and and "AND" or "OR", #state.active_tags)
  end
  set_title("tags", tag_title, state.mode == "tags")

  local list_title = string.format(" Projects  %d/%d ",
...
local function refresh()
  ...
  state.filtered = proj_mod.filter(
    state.projects, state.query, state.active_tags,
    state.cfg.sort_by, state.tag_and)
...
local function refocus_physnav()
  if is_open() then pcall(api.nvim_set_current_win, state.wins.list) end
end
...
local function focus_search()
  ...
  if not (w and b and api.nvim_win_is_valid(w) and api.nvim_buf_is_valid(b)) then return end
  state.mode = "normal"
...
local function on_search_text_changed()
  if SEARCH_RENDER_GUARD then return end
  if not is_open() then return end
  local b = state.bufs.search
  if not (b and api.nvim_buf_is_valid(b)) then return end
...
local function do_tag_edit()
  ...
  vim.ui.input({
    ...
    if is_open() then pcall(api.nvim_set_current_win, state.wins.list); refresh() end
  end)
end
...
local function do_new_note()
  ...
  local function step3_clone()
    ...
    if vim.fn.isdirectory(dest) == 1 then
      ...
      if is_open() then pcall(api.nvim_set_current_win, state.wins.list) end; return
    end
          ...
          vim.fn.jobstart({ "rm", "-rf", dest .. "/.git" }, {
            on_exit = function(_, _)
              vim.schedule(function()
                ...
                state.all_tags = proj_mod.all_tags(state.projects)
                reset_tag_colors()
                vim.notify("PhysNav: created " .. category .. "/" .. name,
                           vim.log.levels.INFO)
                refresh()
                for i, p in ipairs(state.filtered) do
...
local function attach_keymaps(buf, pane)
  ...
  nmap("gg", function()
    if state.mode == "tags" then state.tag_cursor = 0
    else state.selected = 1 end
    refresh()
  end)
  nmap("G",  function()
    if state.mode == "tags" then
  ...
  nmap("h", function()
    if pane == "list"    then focus_tags()
    elseif pane == "preview" then focus_list()
    elseif pane == "tags"    then return  -- already leftmost
    elseif pane == "hints"   then focus_list()
    end
  end)
  nmap("l", function()
    if pane == "tags"    then focus_list()
    elseif pane == "list"    then focus_preview()
    elseif pane == "preview" then return  -- already rightmost
    elseif pane == "hints"   then focus_list()
    end
  end)
  ...
  nmap("<Left>",  function()
    if pane == "list"    then focus_tags()
    elseif pane == "preview" then focus_list() end
  end)
  nmap("<Right>", function()
    if pane == "tags"    then focus_list()
    elseif pane == "list"    then focus_preview() end
  end)
  ...
  nmap("<CR>", function()
    if state.mode == "tags" then
      if state.tag_cursor == 0 then
        select_all_tags(); state.mode = "normal"; focus_list()
      else
  ...
  nmap("a", function()
    if state.tag_multi and #state.active_tags > 1 then
      ...
      vim.notify("AND/OR only applies with multi-select (press m)",
                 vim.log.levels.INFO)
    end
  end)
  ...
  nmap("<C-c>", function()
    state.query = ""; state.active_tags = {}
    state.mode  = "normal"; state.tag_and = false
    state.selected = 1
    refresh()
  end)
  ...
  nmap("e", function() vim.schedule(do_tag_edit) end)
...
local function make_pane(pane, geom, title)
  ...
  vim.wo[win].winhighlight   =
    "NormalFloat:NormalFloat,FloatBorder:PhysNavBorder,CursorLine:PhysNavCursorLine"
...
function M.open(cfg)
  ...
  smap("n", "j",      function() move_list_selection( 1) end)
  smap("n", "k",      function() move_list_selection(-1) end)
  smap("n", "<Down>", function() move_list_selection( 1) end)
  smap("n", "<Up>",   function() move_list_selection(-1) end)
  -- Same idea in insert mode via <C-j>/<C-k> — lets you filter and
  -- navigate without ever leaving insert.
  smap("i", "<C-j>",  function() move_list_selection( 1) end)
  smap("i", "<C-k>",  function() move_list_selection(-1) end)
  smap("i", "<C-n>",  function() move_list_selection( 1) end)
  smap("i", "<C-p>",  function() move_list_selection(-1) end)
  ...
  smap("i", "<C-u>", function()
    ...
    state.selected = 1
    state.filtered = proj_mod.filter(
      state.projects, "", state.active_tags, state.cfg.sort_by, state.tag_and)
    M._render_tags(); M._render_list(); M._render_preview(); M._render_hints()
    pcall(apply_titles)
    clear_hl("search")
    add_hl("search", "PhysNavSearchPrompt", 0, 0, #SEARCH_PROMPT)
    add_hl("search", "PhysNavSearchHint",   0, #SEARCH_PROMPT, -1)
      ...
      callback = function()
        vim.schedule(function()
          ...
          for _, pn in ipairs(PANES) do
            if state.wins[pn] == cur then return end
          end
...
function M.close()
  ...
  state.wins = {}; state.bufs = {}; state.ns = {}
  for _, pane in ipairs(PANES) do
    local w = wins[pane]
    if w and api.nvim_win_is_valid(w) then
